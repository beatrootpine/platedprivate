...
export default function Nav({ currentPath, go }) {
  ...
  const NavLink = ({ path, label, mobile }) => (
    <button onClick={() => { go(path); if (mobile) setMobileOpen(false) }}
      style={{
        background: currentPath === path ? 'var(--gold-dim)' : 'transparent',
  ...
  return (
    <>
      <nav style={{
        ...
        background: 'rgba(250,250,246,0.92)', backdropFilter: 'blur(20px)',
              ...
              user ? (
                ...
                <div style={{ display: 'flex', gap: 6, marginLeft: 8 }}>
                  <button onClick={() => openAuth('login')} style={{
                    background: 'none', border: '1px solid var(--border)',
                    color: 'var(--text)', padding: '7px 16px', borderRadius: 6,
            ...
            {!loading && !user && (
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <button onClick={() => { openAuth('login'); setMobileOpen(false) }} style={{
                  ...
                  color: 'var(--text)', padding: '12px', borderRadius: 8, fontSize: 14, cursor: 'pointer'
                }}>Sign In</button>
